...
public:
    ...
    /** @fn static void QueryChannelInfo(SignalingQueryChannelInfoParam param, SignalingOptCallback cb)
     * @brief 独立信令 查询频道接口
    ...
    /** @fn static void CallEx(SignalingCallParam param, SignalingOptCallback cb)
     * @brief 独立信令 呼叫接口
     * @note 用于用户新开一个频道并邀请对方加入频道，如果返回码不是200、10201、10202时，sdk会主动关闭频道，标记接口调用失败\n
     * 该接口为组合接口，等同于用户先创建频道，成功后加入频道并邀请对方
     * @param[in] param 呼叫的传入参数
    ...
    /** @fn static void JoinAndAccept(SignalingJoinAndAcceptParam param, SignalingOptCallback cb)
     * @brief 独立信令 加入并接受接口
     * @param[in] param 加入并接受的传入参数
     * @param[in] cb 结果回调见nim_signaling_helper.h，返回SignalingJoinAndAcceptResParam
     * @return void 无返回值
     * 错误码：        200:成功;
     *                 0:接口服务异常;
     *               408:超时;
