...
/** @fn void nim_signaling_call_ex(struct NIMSignalingCallParam* param, nim_signaling_opt_cb_func cb, const void *user_data)
 * @brief 独立信令 呼叫接口
 * @note 用于用户新开一个频道并邀请对方加入频道，如果返回码不是200、10201、10202时，sdk会主动关闭频道，标记接口调用失败\n
 * 该接口为组合接口，等同于用户先创建频道，成功后加入频道并邀请对方
 * @param[in] param 呼叫的传入参数
...
/** @fn void nim_signaling_join_and_accept(struct NIMSignalingJoinAndAcceptParam* param, nim_signaling_opt_cb_func cb, const void *user_data)
 * @brief 独立信令 加入并接受接口
 * @note 用于用户加入频道并接受邀请，如果返回码不是200、10201、10202时，sdk会主动关闭频道，标记接口调用失败\n
 * 该接口为组合接口，等同于用户先加入频道，成功后接受邀请
 * @param[in] param 加入并接受的传入参数
 * @param[in] cb 结果回调见nim_signaling_def.h，其中opt_res_param指向NIMSignalingJoinAndAcceptResParam
 * @param[in] user_data APP的自定义用户数据，SDK只负责传回给回调函数，不做任何处理！
 * @return void 无返回值
 * 错误码：        200:成功;
 *                  0:接口服务异常;
