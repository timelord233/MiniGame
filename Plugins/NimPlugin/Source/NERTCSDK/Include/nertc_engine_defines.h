...
struct NERtcCreateAudioMixingOption {
     ...
     * @endif
     * @if Chinese
     * 乐文件的发送音量，取值范围为 0~200。默认为 100，表示使用文件的原始音量。
...
struct NERtcCreateAudioEffectOption {
    ...
    NERtcCreateAudioEffectOption()
        : loop_count(1), send_enabled(true), send_volume(100), playback_enabled(true), playback_volume(100) {
        memset(path, 0, sizeof(path));
    }
};
...
struct NERtcVideoWatermarkImageConfig {
    ...
    NERtcVideoWatermarkImageConfig()
        : wm_alpha(1.0f), wm_width(0), wm_height(0), offset_x(0), offset_y(0), fps(0), loop(true) {
        memset(image_paths, 0, sizeof(image_paths));
    }
};
